...
export class EmbedProcessor {
  ...
  async handleEmbed(job: Job<EmbedJobData>) {
    ...
    this.logger.logJobStart('embed', job.id, { documentId, chunkIndex, contentLength: content.length });
    ...
    try {
      ...
      this.logger.logJobComplete('embed', job.id, duration, { documentId, chunkIndex, embeddingLength: embedding.length });
